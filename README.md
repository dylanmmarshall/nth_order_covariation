**Extracting higher order interactions with multivariate MacLaurin expansion**

This framework extends pairwise sequence saliency analysis to capture 0th order (one below) and 2nd (one above) interactions.

## Citation

If you use this code, please cite:

```
@software{hor2025,
  author = {Marshall, Dylan},
  title = {Higher-Order Relations: Epistatic Interactions via Multivariate Taylor Expansion},
  year = {2025},
  url = {https://github.com/yourusername/HOR}
}
```

## Related Work

- **Marshall et al.** - Pairwise saliency for contact prediction (Jacobian term)
- **Ekeberg et al. (2013)** - GREMLIN (MRF baseline)
- **Koo et al. (2024)** - Attribution analysis for protein models
- **Jumper et al. (2021)** - AlphaFold

## License

MIT

## Contact

For questions or collaboration: dylanmontanamarshall@gmail.com




